...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public Result<ValueResponse> findByName(String name) throws MyException {
        log.info("执行findByName入参name：{}", name);
        if (redisUtil.get(RedisConstant.REDIS_USER_STRING + name) != null) {
            ...
            log.info("执行findByName出参valueResponse：{}", valueResponse);
        ...
        if (user == null) {
            log.error("执行findByName异常：用户不存在");
    ...
    @Override
    public Result<ValueResponse> register(RegisterDTO registerDTO) throws MyException {
